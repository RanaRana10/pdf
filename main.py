...
background_color = (
    # colors.lightgrey
    colors.lightgreen
)  # Can be changed (e.g., colors.white, colors.Color(0.9, 0.9, 0.9))


# Class to represent a single item
class Item:
    def __init__(self, particulars:str, hsn:str, quantity:int, rate:float):
...
class Invoice:
    def __init__(self, shop_details:dict[str, str], customer_details:str, tax_rate:float=0.1):
    ...
    def add_item(self, particulars:str, hsn:str, quantity:int, rate:float):
...
customer_details = {"name": "Customer Name", "address": "Customer Address"}
...
invoice.add_item(particulars="Milk Product", hsn="3rd", quantity=5, rate=22.2)
...
elements.append(
    Paragraph(f"PAN Number: {invoice.shop_details['pan']}", styles["Normal"])
)
